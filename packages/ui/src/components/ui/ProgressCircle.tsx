...
const ProgressCircleContext = React.createContext<ProgressCircleContextValue | null>(null)

const useProgressCircle = () => {
  const context = React.useContext(ProgressCircleContext)
  if (!context) {
    throw new Error('Progress/ProgressLabel must be used within a ProgressCircle')
  }
  return context
}
...
export const ProgressCircle = React.forwardRef<HTMLDivElement, ProgressCircleProps>(
  ({ children, min = 0, max = 100, variant = 'circle', size = 'md', className, ...props }, ref) => {
    ...
    const { progressChildren, labelChildren, totalValue } = React.useMemo(() => {
    ...
    return (
      ...
      >
        <div ref={ref} className={cn('flex flex-col items-center gap-0.5', className)} {...props}>
          <div
            role="progressbar"
          ...
          >
            ...
            >
              ...
              <circle
                ...
                cy={variant === 'gauge' ? radius + strokeWidth / 2 : diameter / 2}
                r={radius}
                fill="none"
                stroke="currentColor"
...
export const Progress = React.forwardRef<SVGCircleElement, ProgressProps>(
  ({ value, className }, ref) => {
    const { min, max, variant, strokeWidth, radius, svgSize } = useProgressCircle()
    ...
    return (
      <circle
        ...
        strokeDashoffset={strokeDashoffset}
        strokeLinecap="round"
        className={cn('text-foreground transition-all duration-300 ease-in-out', className)}
      />
    )
  },
)
...
export const ProgressLabel = React.forwardRef<HTMLDivElement, ProgressLabelProps>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cn(getProgressLabelClasses(variant, size), className)} {...props}>
        {children}
      </div>
    )
  },
