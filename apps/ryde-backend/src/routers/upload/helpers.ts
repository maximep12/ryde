...
export function generateS3FileName(prefix: string, uploadType: UploadType, extension: string = '.csv') {
...
export async function getUploadsSummaries(uploadIds: string[]) {
  if (uploadIds.length === 0)
    return new Map<string, { total: number; valid: number; invalid: number; isProcessed: boolean }>()
...
export async function getUploadIdsByValidationStatus(userId: string, validationStatus: 'valid' | 'invalid') {
...
export async function getUploadResultsCount(uploadId: string, filter?: 'all' | 'valid' | 'invalid') {
  const conditions = [eq(uploadsResults.uploadId, uploadId)]
