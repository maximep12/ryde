...
export const uploadDataDefinition = uploadData
  ...
  .get('/status', zValidatorThrow('query', z.object({ fileName: z.string().optional() })), async (c) => {
    ...
    if (isProcessing) return c.json({ metadata, status: UPLOAD_REPORT_STATUS.PROCESSING, uploadResults })
  ...
  .get('/status/summary', zValidatorThrow('query', z.object({ fileName: z.string().optional() })), async (c) => {
    const { fileName } = c.req.valid('query')
  ...
  .get('/my-uploads', zValidatorThrow('query', myUploadsQueriesSchema), async (c) => {
    ...
    const [uploads, [rowCount]] = await Promise.all([
      getMyUploads(userId, where, queries.page, queries.sort, queries.pageSize ?? PAGE_SIZE, validationFilterIds),
